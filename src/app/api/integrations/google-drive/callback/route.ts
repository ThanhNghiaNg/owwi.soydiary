...
type StoredState = { state?: string; userId?: string; codeVerifier?: string };
...
function popupResponse(status: PopupStatus, message?: string) {
  ...
  const payload = JSON.stringify({
    type: "soyplay:google-drive-oauth",
    status,
    message,
  }).replace(/</g, "\\u003c");
  const copy =
    status === "connected"
      ...
      : "Google Drive connection failed. Please return to Soyplay and try again.";
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (oauthError) return popupResponse("error", oauthError);
    ...
    if (stored.state !== state || stored.userId !== actor.id || !stored.codeVerifier) {
      return popupResponse("error", "invalid_state");
    }

    const connection = await exchangeGoogleDriveCode(actor.id, code, stored.codeVerifier);

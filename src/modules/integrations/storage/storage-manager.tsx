...
export function StorageManager({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  useEffect(() => { if (!open) return; const listener = (event: MessageEvent) => { if (event.origin !== window.location.origin) return; const data = event.data as { type?: string; status?: string }; if (data.type !== "soydiary:cloudinary-oauth" && data.type !== "soydiary:google-drive-oauth") return; if (data.status === "connected") void load(); else setError("Kết nối storage chưa thành công. Bạn hãy thử lại."); }; window.addEventListener("message", listener); return () => window.removeEventListener("message", listener); }, [open, load]);
  useEffect(() => { if (!open) return; const focusTimer = window.setTimeout(() => closeRef.current?.focus(), 0); const keydown = (event: KeyboardEvent) => { if (event.key === "Escape") onClose(); }; window.addEventListener("keydown", keydown); return () => { window.clearTimeout(focusTimer); window.removeEventListener("keydown", keydown); }; }, [open, onClose]);
  if (!open) return null;
  ...
  function connect(provider: "cloudinary" | "google-drive") { setError(""); const popup = window.open(`/api/integrations/${provider}/connect`, `soydiary-${provider}`, "popup,width=620,height=760"); if (!popup) setError("Trình duyệt đang chặn cửa sổ kết nối. Hãy cho phép popup rồi thử lại."); }
  return <div className="dialog-backdrop fixed inset-0 z-[80] grid place-items-center bg-black/45 p-4" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section role="dialog" aria-modal="true" aria-labelledby="storage-title" className="dialog-panel max-h-[min(760px,90dvh)] w-full max-w-lg overflow-y-auto rounded-[1.75rem] bg-white p-5 shadow-2xl">
      <div className="flex items-start justify-between gap-4"><div><h2 id="storage-title" className="text-xl font-black">Quản lý storage</h2><p className="mt-1 text-sm text-[var(--color-muted)]">Chọn nơi lưu hình ảnh của nhật ký.</p></div><button ref={closeRef} type="button" onClick={onClose} aria-label="Đóng" className="grid h-11 w-11 place-items-center rounded-xl bg-[#f2eff5] text-xl">×</button></div>

...
export function BottomNav() {
  ...
  return <nav aria-label="Điều hướng chính" className="safe-bottom fixed inset-x-0 bottom-0 z-50 mx-auto flex w-full min-w-[300px] max-w-[620px] border-t border-[var(--color-border)] bg-white/95 px-3 pt-2 backdrop-blur-xl">
    {tabs.map(({ href, label, Icon }) => { const active = href === "/app" ? pathname === href : pathname.startsWith(href); return <Link aria-current={active ? "page" : undefined} key={href} href={href} className={`flex min-h-14 flex-1 flex-col items-center justify-center gap-0.5 rounded-xl text-xs font-bold transition-colors duration-200 ${active ? "bg-[var(--color-primary-soft)] text-[var(--color-primary-strong)]" : "text-[var(--color-muted)] hover:bg-zinc-50"}`}><Icon className="h-6 w-6"/><span>{label}</span></Link>; })}
  </nav>;
}
